              ...
              <tr key={p._id} className="group hover:bg-white/[0.02] transition-colors relative">
                <td className="px-8 py-10">
                   <div className="flex items-center gap-6">
                      <div className="w-16 h-16 rounded-3xl bg-slate-900 border border-slate-800 flex items-center justify-center overflow-hidden">
                        <img 
                            src={Array.isArray(p.image) ? `http://127.0.0.1:3000/uploads/${p.image[0]}` : `http://127.0.0.1:3000/uploads/${p.image}`} 
                            alt={p.name} 
                ...
                <td className="px-8 py-10 text-right">
                  <div className="flex justify-end gap-3">
                     <button className="px-6 py-3 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 text-white font-black text-[10px] uppercase tracking-widest transition-all">
                        Modify
                     </button>
                     <button 
                        onClick={() => deleteProduct(p._id)}
                        className="px-6 py-3 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-rose-500 hover:bg-rose-500 hover:text-white font-black text-[10px] uppercase tracking-widest transition-all"
                    >
                        Purge
                     </button>
                  </div>
